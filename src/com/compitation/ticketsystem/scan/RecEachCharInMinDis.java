...
public class RecEachCharInMinDis {
	...
	public static Bitmap CreateBmp(int ALeft, int ARight, int ATop,
			int ABottom, Bitmap ABmp) {
	...
	public static Bitmap GetRegion(Bitmap Bmp) {
		...
		int X = 0, Y = 0, Top = Bmp.getHeight(), Bottom = -1, Left = Bmp
				.getWidth(), Right = -1;
				...
				for (FX = 0; FX < W; FX++) {
					...
					for (FI = -1; FI <= 1; FI++) {
						for (FJ = -1; FJ <= 1; FJ++) {
							...
							FX0 = FX + FI + 1;
							FY0 = FY + FJ + 1;
							if ((FX0 < 1) || (FX0 > W) || (FY0 < 1)
									|| (FY0 > H))
								continue;
							if (AMinDis[FY0 - 1][FX0 - 1] == -1)
								continue;
							if (FBmp.getPixel(FX0 - 1, FY0 - 1) != RGBToColor(
									FDisColor - 40, FDisColor - 40,
									FDisColor - 40))
								continue;
							FDisTry = Math.abs(FI) + Math.abs(FJ)
									+ AMinDis[FY0 - 1][FX0 - 1];
							if (FMindisTry >= FDisTry)
								FMindisTry = FDisTry;
						}
					}
					if (FMindisTry < 1000) {
						AMinDis[FY][FX] = FMindisTry;
						FFlag = false;
						FBmp.setPixel(FX, FY,
								RGBToColor(FDisColor, FDisColor, FDisColor));
					}
				}
			}
		}
	...
	public static String XDisRec(int AZFNum, String AFName, String[] AE,
			Bitmap ABmp) {
		...
		MinDisN1 mdn = GetJieShiDis(ABmp, MinDis, N1);
		MinDis = mdn.MinDis;
		N1 = mdn.N1;
		MinDisN1 mdn1 = null;
		Bitmap ocrBmp = null;
		...
		for (I = 0; I <= AZFNum; I++) {
			ocrBmp = AssetsResource.getBitmap(AFName + String.valueOf(I)
					+ ".bmp");
			mdn1 = GetJieShiDis(ocrBmp, PPMindis, N2);
			PPMindis = mdn1.MinDis;
			N2 = mdn1.N1;
			PiPeiDis1 = 0;
			PiPeiSub = 0;
			PiPeiDis2 = 0;
			...
			for (Y = 0; Y < H; Y++) {
				for (X = 0; X < W; X++) {
					...
					if (oriRGB.iB == 255 && ocrRGB.iB == 255)
						continue;
					if (ocrRGB.iB == 0)
						PiPeiDis2 = PiPeiDis2 + MinDis[Y][X];
					if (oriRGB.iB == 0)
						PiPeiDis1 = PiPeiDis1 + PPMindis[Y][X];
				}
			}
			...
			for (Y = 0; Y < H; Y++) {
				for (X = 0; X < W; X++) {
					...
					if (ocrRGB.iB == 255)
						continue;
					PiPeiSub = PiPeiSub + (MinDis[Y][X] - PiPeiDis)
							* (MinDis[Y][X] - PiPeiDis);
				}
			}
			PiPeiSub = PiPeiSub / N1;
			PiPeiDis = PiPeiSub;
			if (PiPeiMin > PiPeiDis) {
				PiPeiMin = PiPeiDis;
				PiPeiNum = I;
			}
		}
	...
	public static String MinDisRec(int AZFNum, String AFName, String[] AE,
			Bitmap ABmp) {
	...
	public static String Math(Bitmap[] bitmaps) {
		...
		if (!HZ1.equals(HZ2) && !HZ1.equals("新") && !HZ2.equals("新")) {
			CPH = "川" + ZM + "•" + SZ1 + SZ2 + SZ3 + SZ4 + SZ5
					+ PlateNumberGroup.CPYS;
			CPH = CPH + "," + HZ2 + ZM + "•" + SZ1 + SZ2 + SZ3 + SZ4 + SZ5
					+ PlateNumberGroup.CPYS;
			CPH = CPH + "," + HZ1 + ZM + "•" + SZ1 + SZ2 + SZ3 + SZ4 + SZ5
					+ PlateNumberGroup.CPYS;
		} else {
			CPH = HZ1 + ZM + "•" + SZ1 + SZ2 + SZ3 + SZ4 + SZ5
					+ PlateNumberGroup.CPYS;
		}
