...
public class CutFrame {
	...
	public static int Accum(Bitmap ABmp, int[][] ASign, int AX, int AY,
			int AAccumValue) {

		AAccumValue = AAccumValue + 1;
		ASign[AX][AY] = 1;
		if ((AX + 1 < ABmp.getWidth())
				&& (ABmp.getPixel(AX + 1, AY) == Color.BLACK)
				&& (ASign[AX + 1][AY] == 0)) {
	...
	public static Bitmap Math(int PlateX, int PlateY, Bitmap oriBmp) {
		...
		Bitmap bitmap = Bitmap.createBitmap(PlateX, PlateY,
