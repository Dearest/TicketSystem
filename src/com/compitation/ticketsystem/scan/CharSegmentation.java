...
public class CharSegmentation {
	...
	public static Bitmap Math(int PlateX, int PlateY, Bitmap oriBmp) {
			...
			if (Math.abs(GroupX[X] - MinJump) < 5) {
				if (X - KN > 4)
					DrawP = 1;
						...
						&& (Math.abs(GroupX[X] - GroupX[X - 1]) >= 1020))
					DrawP = 1;
				if (DrawP == 1) {
					if (TX == 0) {
						if (X < 6)
							...
							if (X > 15) {
								if (TX > (X - TX - 4) / 2) {
									...
									K++;
									PlateNumberGroup.PlateXState[K] = TX;
									JG = X - TX - 4;
									TX = X;
								}
							}
							...
							K++;
							PlateNumberGroup.PlateXState[K] = X;
							TX = X;
						}
					} else {
						if (JG == 0) {
		...
		if (PlateX - PlateNumberGroup.PlateXState[K] > 10) {
			K++;
			PlateNumberGroup.PlateXState[K] = PlateX;
		}
