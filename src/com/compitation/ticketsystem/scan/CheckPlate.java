...
public class CheckPlate {
	...
	public static Bitmap Math(int XMin, int YMin, int XMax, int YMax,
			Bitmap tagBmp) {
		...
		for (Y = 0; Y < H; Y++) {
			...
			for (X = 0; X < W; X++) {
				...
				oriR = Color.red(oriPixColor);
				oriG = Color.green(oriPixColor);
				oriB = Color.blue(oriPixColor);
				oriPixColor1 = oriDst[oriPos + 1];
				oriR1 = Color.red(oriPixColor1);
				oriG1 = Color.green(oriPixColor1);
				oriB1 = Color.blue(oriPixColor1);

				tagPos = Y * W + X;
				tagPixColor = tagDst[tagPos];
				tagR = Color.red(tagPixColor);
				tagG = Color.green(tagPixColor);
				tagB = Color.blue(tagPixColor);

				JumpColorL = (int) Math.round(oriR * 0.3f + oriG * 0.59f + oriB
						* 0.11f);
				JumpColorR = (int) Math.round(oriR1 * 0.3f + oriG1 * 0.59f
						+ oriB1 * 0.11f);
				...
				if (Math.abs(JumpColorL - JumpColorR) > 20)
					YJump++;
			}
			if (YJump > 10)
				JNum++;
		...
		if (JNum > 8) {
			...
			PlateX = bitmap.getWidth();
			PlateY = bitmap.getHeight();
			...
			if ((hingPlateXState > 6) && (hingPlateXState <= 12))
